    ...
    for line in lines:
        ...
        if len(line.strip()) == 0:
            ...
            if empty and cur_emt != 0:
                ...
                for i in range(cur_emt):
                    err.write(f"Empty string error in {ind-cur_emt+i} line\n")
            cur_emt = 0
            empty = False

        ind += 1
    for i in range(cur_emt):
        err.write(f"Empty string error in {len(lines)-cur_emt+i+1} line\n")
...
def main():
    ...
    check_empty_lines("test.txt", err, 2, 1)
    check_tabs("test.txt", err, 1)
    check_space("test.txt", err, space_elements)
    check_lines_len("test.txt", err, max_len)
